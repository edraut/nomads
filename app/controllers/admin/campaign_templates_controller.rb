class Admin::CampaignTemplatesController < Admin::AdministrationController
  ...
  def create
    ...
    respond_to do |format|
      if @campaign_template.save
        ...
        format.html { redirect_to([:admin,@campaign_template]) }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to(admin_campaign_templates_url) }
      format.xml  { head :ok }
    end
  end
end
